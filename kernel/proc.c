...
exit(int status,char* msg)
{
  ...
  wakeup(p->parent);
  // argstr(0,p->exit_msg,strlen(msg));
  // char *msg_copy="";
  // argstr(0, msg,strlen(msg)); // Copy string from user space to kernel space


  // safestrcpy(p->exit_msg, msg, sizeof(p->exit_msg)); // Copy string to process PCB


    // ... rest of the exit() function code
...
wait(uint64 addr,uint64 dst)
{
  ...
  for(;;){
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          // printf("kernel space message: %s\n", pp->exit_msg);
          //copyout-kernel space to user 
    ...
    // printf("hi\n");
    // char copymsg[32];
    // if (copyout(p->pagetable,(uint64)copymsg,msg,strlen(msg))<0){
    //   printf("error\n");
    // }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // struct proc *min_proc=(help()) ;
    // Switch to the process with the lowest accumulator value.
    if(min_proc!=0){
      // printf("no");
